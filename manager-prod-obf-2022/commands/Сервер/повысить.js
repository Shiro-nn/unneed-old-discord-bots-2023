        ...
        try{
            if(message.channel.id === '747879895316824104'){
                ...
                if(adm.sl.trainee){
                    ...
                    adm.markModified('sl');
                    await adm.save();
                    member.roles.remove('699234064699490306');
                    member.roles.add('623870779972517918');
                    member.roles.add('1170367561360216246'); // отряд

                    let messageOptions = {};
                    messageOptions.embed = new Discord.MessageEmbed().setDescription(`Поздравляю:tada:\nВы повышены, ваша новая роль: Хелпер ${client.config.agree}`).setColor('#00ffff');
                    ...
                    let messageOptions = {};
                    messageOptions.embed = new Discord.MessageEmbed().setDescription(`Поздравляю:tada:\nВы повышены, ваша новая роль: Главный Хелпер ${client.config.agree}`).setColor('#0089c7');
                    member.send(messageOptions);
                    isadm = true;
                    ...
                    let messageOptions = {};
                    messageOptions.embed = new Discord.MessageEmbed().setDescription(`Поздравляю:tada:\nВы повышены, ваша новая роль: Админ ${client.config.agree}`).setColor('#fdffbb');
                    member.send(messageOptions);
                    isadm = true;
                    ...
                    member.roles.add('653908539781152768');

                    let messageOptions = {};
                    messageOptions.embed = new Discord.MessageEmbed().setDescription(`Поздравляю:tada:\nВы повышены, ваша новая роль: Главный Админ ${client.config.agree}`).setColor('#ff0000');
                    member.send(messageOptions);
                    isadm = true;
                }
                if(isadm){
                ...
                .setColor('#15ff00')
                await message.guild.channels.cache.get('745653433180356609').send(messageOpt);
