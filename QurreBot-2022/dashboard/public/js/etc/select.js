...
const pureScriptSelect = (selector) => {
    ...
    selectors.forEach((item, index) => {
        const atrDesc = item.getAttribute('data-message');
        ...
        function singleSelect(){
            ...
            <div class="directorist-select__dropdown">
                <input class='directorist-select__search ${ isSearch ? 'directorist-select__search--show' : 'directorist-select__search--hide' }' type='text' class='value' placeholder='${atrDesc}' />
        ...
        function StringToArray(string){
            try{return string.split(',');}catch{return []}
        }
        function multiSelects(){
            let defaultValues = {
                [document.querySelector(selector).getAttribute('id')]: StringToArray(document.querySelector(selector).getAttribute('data-default'))
            };
            const arraySelector = item.getAttribute('id');            
            ...
            <div class="directorist-select__label">
                ...
                <input type="text" class='directorist-select__search ${ isSearch ? 'directorist-select__search--show' : 'directorist-select__search--hide' }' type='text' class='value' placeholder='${atrDesc}' />
            </div>
            <div class="directorist-select__dropdown">            
                <div class="directorist-select__dropdown--inner"></div>
            </div>
            ...
            function insertSearchItem () {
                var list = [];
                defaultValues[arraySelector].forEach(async ch => {
                    let qq = optionValues[arraySelector].find(x => x.id == ch);
                    if(qq != undefined && qq != null) list.push(qq);
                });
                item.querySelector('.directorist-select__selected-list').innerHTML = list.map(item => `<span class="directorist-select__selected-list--item">${item.name}&nbsp;&nbsp;<a href="#" data-key="${item.id}" class="directorist-item-remove"><i class="fa fa-times"></i></a></span>`).join("")
            }
            ...
            button.addEventListener('click', (e) => {
                ...
                option.forEach((el, key) => {  
                    item2 += `<li data-key="${el.id}" class="directorist-select-item-hide">${el.name}</li>`;
                });
                item2 += '</ul>';
                ...
                value && value.addEventListener('keyup', (event) => {     
                    ...
                    var itemValue = event.target.value.toLowerCase();
                    var filter = option.filter((el, index) => el.name.toString().toLowerCase().startsWith(itemValue));
