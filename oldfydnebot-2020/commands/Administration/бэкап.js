    ...
    async run (message, args, data) {
        ...
        if(status === "создать"){
            ...
            backup.create(message.guild, {
                ...
                message.author.send(message.language.get("BACKUP_CREATE_SUCCESS_ID", backupData.id));
                message.channel.send(message.language.get("BACKUP_CREATE_SUCCESS"));
            });
        } else if (status === "загрузить"){
            let backupID = args[1];
            if(!backupID){
            ...
            backup.fetch(backupID).then(async () => {
                ...
                message.channel.send(message.language.get("BACKUP_CONFIRMATION"));
                    await message.channel.awaitMessages(m => (m.author.id === message.author.id) && (m.content === "-подт"), {
                        max: 1,
                        time: 20000,
                        errors: ["time"]
                    }).catch((err) => {
