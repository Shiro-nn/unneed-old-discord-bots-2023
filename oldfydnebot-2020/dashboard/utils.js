...
async function fetchUser(userData, client, query){
    if(userData.guilds){
        userData.guilds.forEach((guild) => {
            ...
            guild.iconURL = (guild.icon ? `https://cdn.discordapp.com/icons/${guild.id}/${guild.icon}.png?size=128` : "/discord.png");
            guild.settingsmsg = (client.guilds.get(guild.id) ? `Настроить` : `Пригласить`);
            guild.statsbool = (client.guilds.get(guild.id) ? true : false);
