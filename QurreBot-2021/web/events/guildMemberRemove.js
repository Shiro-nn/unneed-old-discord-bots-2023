    ...
    async run(member) {
        try {
            ...
            TicketLogs.filter(x => x.user == member.id && x.guild == guild.id).forEach(async log => {
                ...
                exec(`dotnet ${config.directory}/DiscordChatExporter.CLI/DiscordChatExporter.Cli.dll export -t "${config.token}" -b -c ${channel.id} -f HtmlDark -o "${config.root_directory}/tickets/${channel.id}/index.html"`, async (error) => {
