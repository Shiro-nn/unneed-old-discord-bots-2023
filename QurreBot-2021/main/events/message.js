...
module.exports = class {
    ...
    async run (message) {
        ...
        if(message.guild){
            let _member = await this.client.database.Members.findOne({id: message.author.id, guild: message.guild.id});
            if (!_member) _member = await this.client.database.Members({id: message.author.id, guild: message.guild.id }).save();
            data.member = _member;
