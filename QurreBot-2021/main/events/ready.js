module.exports = class {
    ...
    async run () {
        ...
        setInterval(()=>{
            let gsh = shid;
            gsh++;
            let toDisplay = `${status[parseInt(i, 10)].name} | Shard: ${gsh}`;
            client.user.setPresence({ activity: { name: toDisplay, type: status[parseInt(i, 10)].type }, status: "dnd", shardID: [shid] });
