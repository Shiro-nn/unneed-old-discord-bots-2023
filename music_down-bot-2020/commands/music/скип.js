...
class skip extends Command {
    ...
    async run (message, args, data) {
        ...
        if(args[0] > queue.songs.length){
            messageOptions.embed = new Discord.MessageEmbed().setDescription(`\\💢 | В плейлисте только ${queue.songs.length} треков`).setColor(data.config.embed.color);
            let m = await message.reply(messageOptions).catch();
            setTimeout(() => {
                m.delete().catch();
            }, 5000);
        ...
        messageOptions.embed = new Discord.MessageEmbed().setDescription(`\\⏭ | ${message.author} пропустил ${args[0] - 1} треков`).setColor(data.config.embed.color);
