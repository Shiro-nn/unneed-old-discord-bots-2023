...
class playlist extends Command {
    ...
    async run (message, args, data) {
        ...
        if(!args.length){
            let m = await message.reply(`пример: ${data.config.prefix}плейлист <YouTube URL | Название>`).catch();
            setTimeout(() => {
                m.delete().catch();
            }, 5000);
            return;
        ...
        if(urlValid){
            try{
                ...
                let messageOptions = {};
                messageOptions.embed = new Discord.MessageEmbed().setDescription(`\\💢 | Плейлист не найден :(`).setColor(data.config.embed.color);
            ...
            if(args[0].includes("/sets/")){
                let messageOptions = {};
                messageOptions.embed = new Discord.MessageEmbed().setDescription(`\\⌛ | Получение плейлиста...`).setColor(data.config.embed.color);
                let m = await message.reply(messageOptions);
                setTimeout(() => {
                    m.delete().catch();
                }, 5000);
