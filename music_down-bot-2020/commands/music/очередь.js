...
class queue extends Command {
    ...
    async run (message, args, data) {
        ...
        if(!serverQueue){
            messageOptions.embed = new Discord.MessageEmbed().setDescription(`\\❌ | На этом сервере ничего не играет`).setColor(data.config.embed.color);
        ...
        try{
            ...
            collector.on("collect", async (reaction, user) => {
                try{
                    ...
                    messageOptions.embed = new Discord.MessageEmbed().setDescription(`\\❌ | **Нужны права - [ADD_REACTIONS, MANAGE_MESSAGES]!**`).setColor(data.config.embed.color);
                    let m = await message.channel.send(messageOptions).catch();
                    setTimeout(() => {
                        m.delete().catch();
                    }, 5000);
